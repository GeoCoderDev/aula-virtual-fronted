...
const Curso = ({ params: { CursoAula_ID } }: { params: { CursoAula_ID: number } }) => {
  ...
  useEffect(() => {
    const { status, messageError } = validateCourseId(CursoAula_ID);
  ...
  return (
    <div className="flex flex-col gap-y-1 w-full">
      ...
      {!errorCursoID && !isSomethingLoading && (
        <CourseData idCourseClassroom={CursoAula_ID} />
      )}
    </div>
  );
};
