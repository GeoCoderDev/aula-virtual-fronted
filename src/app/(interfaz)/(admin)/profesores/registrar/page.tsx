...
const RegistrarProfesor = () => {
  ...
  const {
    ...
    csvData,
    setCsvData,
  } = useCSVRegister<TeacherRegisterForm>(initialForm);
  ...
  const registrarProfesor = async () => {
    ...
    try {
      ...
      //Validando el DNI
      const { status: statusDNI } = validateDNI(form.DNI_Profesor, setError);
      if (!statusDNI) return;
  ...
  const registrarProfesores = async () => {
    try {
      ...
      const fetchCancelable = fetchAPI(
        "/api/teachers/multiple",
        "POST",
        null,
        JSON.stringify({ teacherValues: csvData })
      );
  ...
  return (
    <div className="w-full flex flex-col">
      <div className="flex flex-wrap w-full justify-start gap-6 items-center">
        <h1 className="section-tittle">Registrar Profesor</h1>
        <button
          onClick={() => {
      ...
      >
        {registerWithCSV ? (
          <UploadCSVForm<TeacherRegisterForm>
            HelpComponent={(props) => {
              return <HelpCSVTeacherRegister {...props} />;
            }}
            ...
            columnsUnique={["DNI_Profesor", "Nombre_Usuario"]}
            setError={setError}
          />
        ) : (
          <RegisterTeacherForm
